  ...
  return (
    <main className="min-h-screen px-4 py-10 text-white sm:px-6 lg:px-8">
      <section className="mx-auto flex max-w-6xl flex-col gap-8">
        ...
        <ChartCard title="Weekly peaks" subtitle="Commit count by day" status={status} error={error}><PageSection eyebrow="Paris" title="Weekly activity"><div className="h-72 w-full"><ActivityBarChart data={days.map((day, i) => ({ day, commits: dayData[i] }))} xKey="day" yKey="commits" barRadius={[8, 8, 0, 0]} colorOffset={1} margin={{ top: 10, right: 10, left: 0, bottom: 0 }} /></div><div className="mt-4"><InsightCard label="Best day" value={days[bestIndex(dayData)]} detail={`${Math.max(...dayData)} commits`} /></div></PageSection></ChartCard>

        <ChartCard title="Message themes" subtitle="Representative categories from commit messages" status={status} error={error}><PageSection eyebrow="Themes" title="Message categories"><div className="h-[30rem] w-full"><ActivityBarChart data={sortedThemes} layout="vertical" xKey="commits" yKey="theme" xAxisProps={{ type: 'number' }} yAxisProps={{ width: 210, interval: 0, tick: { width: 210, textAnchor: 'end' } }} barRadius={[0, 8, 8, 0]} colorOffset={2} margin={{ top: 10, right: 20, left: 210, bottom: 0 }} /></div><div className="mt-4 grid grid-cols-2 gap-4"><InsightCard label="Top theme" value={sortedThemes[0].theme} detail={`${sortedThemes[0].commits} commits`} /><InsightCard label="Bottom theme" value={sortedThemes[sortedThemes.length - 1].theme} detail={`${sortedThemes[sortedThemes.length - 1].commits} commits`} /></div></PageSection></ChartCard>
