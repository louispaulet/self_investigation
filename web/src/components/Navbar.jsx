...
const links = [
  ...
  { to: '/average-hour', label: 'Average Hour' },
...
export default function Navbar() {
  return (
    <header className="sticky top-0 z-20 border-b border-white/10 bg-[#07110f]/80 px-4 py-3 text-white shadow-lg shadow-black/10 backdrop-blur sm:px-6 lg:px-8">
      <nav className="mx-auto flex max-w-7xl flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
        ...
        <div className="flex w-full flex-wrap items-center gap-1 rounded-xl border border-white/10 bg-black/15 p-1 sm:w-auto sm:max-w-[calc(100vw-5.5rem)] sm:flex-nowrap sm:overflow-x-auto">
          {links.map((link) => (
            <NavLink
              key={link.to}
              to={link.to}
